...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(entries => {
      ...
      root.style.setProperty('--segments-x', segments.toString());
      root.style.setProperty('--segments-y', segments.toString());
      
      applyTransform(rotationRef.current.x, rotationRef.current.y);
    });
    ro.observe(root);
    return () => ro.disconnect();
  }, [fit, fitBasis, minRadius, maxRadius, padFactor, overlayBlurColor, grayscale, imageBorderRadius, openedImageBorderRadius, segments]);
  ...
  const openItem = useCallback((e: React.MouseEvent) => {
    if (movedRef.current || performance.now() - lastDragEndAt.current < 150) return;
    ...
    overlay.style.cssText = `width: ${openedImageWidth}; height: ${openedImageHeight};`;
    const newImg = document.createElement('img');
    newImg.src = img.src;
    overlay.appendChild(newImg);
    viewerRef.current?.appendChild(overlay);
  }, [lockScroll, segments, openedImageWidth, openedImageHeight]);
        ...
        <div className="stage">
          <div ref={sphereRef} className="sphere">
            {items.map((item, i) => (
              ...
              >
                <div className="item__image" onClick={openItem}>
